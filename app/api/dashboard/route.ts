...
export async function GET() {
  try {
    ...
    // Count transactions per rep (by acq rep)
    transactions.forEach(trans => {
    ...
    repData.forEach((data, ownerId) => {
      ...
      const stalePercent = data.leadsActive > 0 ? (data.leadsStale7d / data.leadsActive) * 100 : 0;
      const daysSinceActivity = data.lastActivity 
        ? daysBetween(data.lastActivity, now) 
        : 999;
      
      // Only include reps with some activity
    ...
    // Check for reps with low activity this week
    ...
    const dashboard: DashboardData & { blockedTransactions: BlockedTransaction[] } = {
      leads: {
